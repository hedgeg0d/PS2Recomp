...
namespace ps2recomp
{
    ...
    void PS2Recompiler::generateOutput()
    {
        try
        {
            ...
            // Consumers must not glob the output directory: a renamed or
            // removed function can leave an older translation there. Keep
            // those files intact and publish the exact current source set.
            std::ostringstream sourceManifest;
            sourceManifest << "# Generated by PS2Recomp. Do not edit.\n"
                           << "set(PS2_RECOMPILED_SOURCES\n"
                           << "    \"${CMAKE_CURRENT_LIST_DIR}/register_functions.cpp\"\n";
            if (m_config.singleFileOutput)
            {
                sourceManifest << "    \"${CMAKE_CURRENT_LIST_DIR}/ps2_recompiled_functions.cpp\"\n";
            }
            else
            {
                for (const Function *function : outputFunctions)
                {
                    sourceManifest << "    \"${CMAKE_CURRENT_LIST_DIR}/"
                                   << getOutputPath(*function).filename().generic_string() << "\"\n";
                }
            }
            sourceManifest << ")\n";
            const fs::path manifestPath = fs::path(m_config.outputPath) / "ps2_recompiled_sources.cmake";
            if (!writeToFile(manifestPath.string(), sourceManifest.str()))
            {
                throw std::runtime_error("Failed to write generated source manifest: " + manifestPath.string());
            }
        }
        catch (const std::exception &e)
        {
    ...
    void PS2Recompiler::discoverAdditionalEntryPoints()
    {
        ...
        for (const auto &function : m_functions)
        {
            ...
            // A mapped range can contain a compact leaf helper immediately
            // after another return instruction.  Such helpers are commonly
            // installed through data-driven callback tables, so their JR $ra
            // entry is not visible to static branch-target discovery.  Keep
            // every internal return instruction address resumable; this lets
            // the generated function be entered at the actual leaf target
            // without a title-specific map split or runtime fallback.
            for (const Instruction &instruction : instructions)
            {
                if (instruction.isReturn && instruction.address != function.start)
                {
                    ownerTargets.push_back(instruction.address);
                }
            }
    ...
    bool PS2Recompiler::decodeFunction(Function &function)
    {
        ...
        // Function maps may split a MIPS instruction from its delay slot at
        // an artificial boundary.  The slot is architecturally part of the
        // branch and must still be decoded/emitted by the owning function;
        // otherwise returns silently skip stack restoration (and similar
        // side effects).  Keep function.end unchanged so registration and
        // entry-point ownership remain stable, but include the physical slot
        // in the instruction stream consumed by FunctionEmitter.
        if (!instructions.empty() && instructions.back().hasDelaySlot)
        {
            const uint32_t delayAddress = instructions.back().address + 4u;
            if (delayAddress >= end && m_elfParser->isValidAddress(delayAddress))
            {
                try
                {
                    uint32_t rawInstruction = m_elfParser->readWord(delayAddress);
                    const uint32_t originalInstruction = rawInstruction;
                    auto patchIt = m_config.patches.find(delayAddress);
                    if (patchIt != m_config.patches.end())
                    {
                        const PatchClass patchClass = classifyPatchedInstruction(originalInstruction);
                        if (shouldApplyConfiguredPatch(patchClass, m_config))
                    ...
                    Instruction delaySlot = m_decoder->decodeInstruction(delayAddress,
                                                                         rawInstruction,
                                                                         !m_config.lowMemoryMode);
                    auto mmioIt = m_config.mmioByInstructionAddress.find(delayAddress);
                    if (mmioIt != m_config.mmioByInstructionAddress.end())
                    {
                        delaySlot.isMmio = true;
                        delaySlot.mmioAddress = mmioIt->second;
                    }
                    instructions.push_back(delaySlot);
                }
                catch (const std::exception &e)
                {
                    std::ostringstream msg;
                    msg << "Error decoding cross-boundary delay slot: " << e.what();
                    m_reporter.warningAt("decode", function.name, delayAddress, msg.str());
                }
            }
        }
    ...
    {
        // Preserve timestamps for unchanged translations and shared headers.
        // Otherwise a no-op export dirties every dependent compilation unit.
        std::ifstream existing(path);
        if (existing)
        {
            char buffer[8192];
            size_t matched = 0;
            while (matched < content.size())
            {
                const size_t count = std::min(sizeof(buffer), content.size() - matched);
                existing.read(buffer, static_cast<std::streamsize>(count));
                if (existing.gcount() != static_cast<std::streamsize>(count) ||
                    std::memcmp(buffer, content.data() + matched, count) != 0)
                {
                    break;
                }
                matched += count;
            }
            if (matched == content.size() && existing.peek() == std::char_traits<char>::eof() && !existing.bad())
            {
                return true;
            }
        }
        ...
        if (!file)
        {
            m_reporter.error("file", "Failed to finish writing: " + path);
            return false;
        }
        return true;
    }
