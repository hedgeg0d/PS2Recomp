...
public class ExportPS2FunctionsHeadless extends GhidraScript {
    ...
    public void run() throws Exception {
        ...
        if (scriptArgs.length < 2) {
            println("Usage: ExportPS2FunctionsHeadless <out.toml> <out.csv>");
            return;
        }
        File tomlFile = new File(scriptArgs[0]);
        File csvFile = new File(scriptArgs[1]);
        ...
        while (it.hasNext() && !monitor.isCancelled()) {
            ...
            record.endExclusive = record.start + Math.max(1L, record.size);
